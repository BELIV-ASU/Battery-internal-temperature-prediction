...
def plot_predictions(preds_C, truths_C, title, save_path, n_show=3000):
    ...
    ax.axhline(HOTSPOT_THRESH, color="#dc2626", lw=0.8, ls="-.",
               alpha=0.6, label="Safety threshold 44C")
    ...
    ax = axes[1]
    error = preds_C[:n_show] - truths_C[:n_show]
    ax.plot(idx, error, color="#7c3aed", lw=0.8, alpha=0.7)
    ax.axhline(0,  color="gray",    lw=0.8)
    ax.axhline( 1, color="#f59e0b", lw=0.8, ls="--", alpha=0.6, label="+1C error")
    ax.axhline(-1, color="#f59e0b", lw=0.8, ls="--", alpha=0.6, label="-1C error")
    ax.set_ylabel("Prediction Error (C)")
    ax.set_xlabel("Sample index")
    ax.set_title("Prediction Error (closer to 0 = better)")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.25)
